...
if($nb_utilisateurs == 0) {
    ...
    if($audit['success'] == true) {
        $edition = $UTILISATEURS->editer($parametres['user_id'],NULL,NULL,$parametres['email'],$parametres['username'],$parametres['firstname'],$parametres['lastname'],$parametres['direction'],$parametres['service'],$parametres['fonction'],$parametres['num_telephone'],$user);
...
<small><i>&copy; Powered By <a href="https://techouse.io">TecHouse</a></i></small>
    ...
    if(isset($_SESSION['auxilium_user_id'])) {
        ...
        if(!empty($utilisateur['user_id'])) {
            $audit = $UTILISATEURS->ajouter_log_piste_audit(NULL,NULL,NULL,NULL);
        }else {
            $json = array(
                'success' => false,
