...
{
    function trouver_liste_fichiers($norme,$num_fichier,$nom_fichier) {
        $a = $this->bdd->prepare('SELECT log_id AS fichier_id, log_mouvement AS mouvement, log_num_fichier AS num_fichier, log_norme AS norme, log_version AS version, log_date_fichier AS fichier_date, log_code_emetteur AS code_emetteur, log_code_destinataire AS code_destinataire, log_occurrences_fichier AS occurrences_fichier, log_nom_fichier AS fichier_nom, date_reg, user_reg FROM auxil_log_historique_fichier WHERE log_norme LIKE ? AND log_num_fichier LIKE ? AND log_nom_fichier LIKE ? ORDER BY date_reg DESC');
        $a->execute(array('%'.$norme.'%','%'.$num_fichier.'%','%'.$nom_fichier.'%'));
        $json = $a->fetchAll();
        return $json;
    }
