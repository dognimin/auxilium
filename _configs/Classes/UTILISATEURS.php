...
class UTILISATEURS extends BDD {
    ...
    public function reinitialiser_mot_de_passe($user_id,$mot_de_passe,$voucher) {
        ...
        $a->execute(array($password, 0, $voucher,$user_id))OR DIE('Erreur Lors de la réinitialisation du mot de passe.');
    ...
    public function editer($id_user, $code_ogd, $num_matricule, $email, $username, $firstname, $lastname, $direction, $service, $fonction, $num_telephone, $user) {
        if(empty($id_user)) {
            $trouver_email = $this->trouver(NULL,$email,NULL);
            if(!$trouver_email) {
                $trouver_pseudo = $this->trouver(NULL,$email,NULL);
                if(!$trouver_pseudo) {
                    ...
                    $a->execute(array(
                        ...
                        'utilisateur_statut' => 1,
                        'user_reg' => $user
                    ))OR DIE('Erreur nouvelle édition');
                    ...
                    $json = array(
                        'success' => false,
                        'message' => 'Ce nom utilisateur email est déjà utilisé par un autre utilisateur. Prière en saisir un autre.'
                    );
                }
            }else {
                $json = array(
